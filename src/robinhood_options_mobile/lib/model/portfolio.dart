...
class Portfolio {
  ...
  Portfolio.fromJson(dynamic json)
    : url = json['url'],
      ...
      extendedHoursMarketValue = parseDouble(
        json['extended_hours_market_value'],
      ),
      extendedHoursEquity = parseDouble(json['extended_hours_equity']),
      extendedHoursPortfolioEquity = parseDouble(
        json['extended_hours_portfolio_equity'],
      ),
      lastCoreMarketValue = parseDouble(json['last_core_market_value']),
      lastCoreEquity = parseDouble(json['last_core_equity']),
      lastCorePortfolioEquity = parseDouble(json['last_core_portfolio_equity']),
      excessMargin = parseDouble(json['excess_margin']),
      excessMaintenance = parseDouble(json['excess_maintenance']),
      excessMarginWithUnclearedDeposits = parseDouble(
        json['excess_margin_with_uncleared_deposits'],
      ),
      excessMaintenanceWithUnclearedDeposits = parseDouble(
        json['excess_maintenance_with_uncleared_deposits'],
      ),
      equityPreviousClose = parseDouble(json['equity_previous_close']),
      portfolioEquityPreviousClose = parseDouble(
        json['portfolio_equity_previous_close'],
      ),
      adjustedEquityPreviousClose = parseDouble(
        json['adjusted_equity_previous_close'],
      ),
      adjustedPortfolioEquityPreviousClose = parseDouble(
        json['adjusted_portfolio_equity_previous_close'],
      ),
  ...
  Portfolio.fromSchwabJson(dynamic json)
    : url = '',
      ...
      marketValue =
          (parseDouble(
                json['securitiesAccount']['currentBalances']['longOptionMarketValue'],
              ) ??
              0) +
          (parseDouble(
                json['securitiesAccount']['currentBalances']['longMarketValue'],
              ) ??
              0),
      equity = parseDouble(
        json['securitiesAccount']['currentBalances']['liquidationValue'],
      ), //double.tryParse(json['equity']),
      ...
      excessMaintenance = null, // double.tryParse(json['excess_maintenance']),
