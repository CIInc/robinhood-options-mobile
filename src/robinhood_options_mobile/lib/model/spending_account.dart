...
class SpendingAccount {
  ...
  factory SpendingAccount.fromJson(dynamic json) {
    ...
    final id =
        (json['id'] ?? json['account_id'] ?? json['uuid'] ?? '').toString();
    final acctNum = (json['account_number'] ??
            json['mask'] ??
    ...
    final bal = parseDouble(json['balance'] ??
            json['current_balance'] ??
            json['total_balance']) ??
        0.0;
    final avail = parseDouble(json['available_balance'] ??
            json['cash_available'] ??
            json['available']) ??
        bal;
    final unsettled =
    ...
    final cType =
        card != null ? card['type']?.toString() : json['card_type']?.toString();
