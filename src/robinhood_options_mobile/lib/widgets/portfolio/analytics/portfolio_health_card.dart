  ...
  static void showDetails(BuildContext context, Map<String, dynamic> data) {
    ...
    if (data.containsKey('sharpe')) {
      ...
      if (sharpe > 2.5) {
        ...
        riskAdjustedScore += 9;
        riskAdjustedDetails.add(
          'Sharpe: ${sharpe.toStringAsFixed(2)} (> 1.0 Good) [+9]',
        );
      } else if (sharpe > 0.5) {
        riskAdjustedScore += 5;
        riskAdjustedDetails.add(
        ...
        riskAdjustedScore -= 5;
        riskAdjustedDetails.add(
          'Sharpe: ${sharpe.toStringAsFixed(2)} (< 0 Negative) [-5]',
        );
      }
    }
    ...
    if (data.containsKey('treynor')) {
      ...
      if (treynor > 0.20) {
        ...
        riskAdjustedScore += 2;
        riskAdjustedDetails.add(
          'Treynor: ${treynor.toStringAsFixed(2)} (> 0.10 Good) [+2]',
        );
      }
    }
    if (data.containsKey('omega')) {
      ...
      if (omega > 2.5) {
        riskAdjustedScore += 5;
        riskAdjustedDetails.add(
          'Omega: ${omega.toStringAsFixed(2)} (> 2.5 Exceptional) [+5]',
        );
      } else if (omega > 1.5) {
        riskAdjustedScore += 3;
        riskAdjustedDetails.add(
          'Omega: ${omega.toStringAsFixed(2)} (> 1.5 Very Good) [+3]',
        );
      } else if (omega < 1.0) {
        riskAdjustedScore -= 5;
        riskAdjustedDetails.add(
          'Omega: ${omega.toStringAsFixed(2)} (< 1.0 More Losses) [-5]',
        );
      }
    }
    ...
    if (data.containsKey('beta')) {
      ...
      if (beta > 1.8) {
        ...
        riskManagementScore -= 3;
        riskManagementDetails.add(
          'Beta: ${beta.toStringAsFixed(2)} (> 1.5 Aggressive) [-3]',
        );
      } else if (beta < 0.3) {
        riskManagementScore -= 4;
        riskManagementDetails.add(
          'Beta: ${beta.toStringAsFixed(2)} (< 0.3 Too Defensive) [-4]',
        );
      }
    }
    ...
    if (data.containsKey('profitFactor')) {
      ...
      if (pf > 3.0) {
        ...
        efficiencyScore += 3;
        efficiencyDetails.add(
          'Profit Factor: ${pf.toStringAsFixed(2)} (> 1.2 Good) [+3]',
        );
      } else if (pf < 0.9) {
        efficiencyScore -= 12;
        efficiencyDetails.add(
    ...
    if (data.containsKey('calmar')) {
      ...
      if (calmar > 2.0) {
        ...
        efficiencyScore += 3;
        efficiencyDetails.add(
          'Calmar: ${calmar.toStringAsFixed(2)} (> 1.0 Good) [+3]',
        );
      } else if (calmar < 0) {
        efficiencyScore -= 3;
        efficiencyDetails.add(
    ...
    if (data.containsKey('expectancy')) {
      ...
      if (exp > 0) {
        ...
        efficiencyScore -= 4;
        efficiencyDetails.add(
          'Expectancy: \$${exp.toStringAsFixed(2)} (≤ 0 No Edge) [-4]',
        );
      }
    }
