...
class _ConnectedAgentsWidgetState extends State<ConnectedAgentsWidget> {
  ...
  void _loadData() {
    setState(() {
      _futureTokens =
          widget.service.getExternalTokensModel(widget.brokerageUser);
    });
  }
  ...
  Future<void> _revokeToken(ExternalToken token) async {
    ...
    if (confirmed == true) {
      final success = await widget.service
          .revokeExternalToken(widget.brokerageUser, token.id);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              ...
              children: [
                ...
                if (token.isActive) ...[
                  FilledButton.icon(
                    ...
                    label: const Text('Revoke Token Access',
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<ExternalToken>>(
        ...
        builder: (context, snapshot) {
          ...
          final oauthCount =
              allTokens.where((t) => !t.isAgent && !t.isAggregator).length;
          ...
          if (_selectedCategory == 'agent') {
            ...
            filtered =
                filtered.where((t) => !t.isAgent && !t.isAggregator).toList();
          }
          ...
          if (_searchQuery.isNotEmpty) {
            ...
            filtered = filtered.where((t) {
              ...
              final accountMatch =
                  t.agenticAccounts.any((a) => a.contains(query));
              return nameMatch ||
                  titleMatch ||
                  subtitleMatch ||
                  agentMatch ||
          ...
          filtered.sort((a, b) {
            switch (_sortOption) {
              ...
              case _SortOption.firstLogin:
                final dateA = a.initialLoginTime ??
                    a.createdAt ??
                    DateTime.fromMillisecondsSinceEpoch(0);
                final dateB = b.initialLoginTime ??
                    b.createdAt ??
                    DateTime.fromMillisecondsSinceEpoch(0);
                return dateB.compareTo(dateA);
              case _SortOption.name:
                return a.primaryTitle
                    .toLowerCase()
                    .compareTo(b.primaryTitle.toLowerCase());
            ...
            child: ListView(
              ...
              children: [
                ...
                if (filtered.isEmpty)
                  ...
                  ...filtered
                      .map((token) => _buildCompactTokenTile(context, token))
                else
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _buildStatBadge(context, '$agents', 'AI Agents',
                    const Color(0xFF7C4DFF), Icons.psychology),
                const SizedBox(width: 8),
                _buildStatBadge(context, '$linked', 'Linked Hubs',
                    const Color(0xFF00897B), Icons.hub_outlined),
                const SizedBox(width: 8),
                _buildStatBadge(context, '$oauth', 'Direct Apps',
                    const Color(0xFF1E88E5), Icons.apps_outlined),
              ],
            ),
            const SizedBox(height: 10),
            InkWell(
  ...
  Widget _buildStatBadge(BuildContext context, String count, String label,
  ...
  Widget _buildToolbar(BuildContext context,
      {required int filteredCount, required int totalCount}) {
    final theme = Theme.of(context);
  ...
  Widget _buildTokenCard(BuildContext context, ExternalToken token) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: token.isAgent
            ? BorderSide(
                color: token.typeColor.withValues(alpha: 0.35), width: 1.5)
            : BorderSide(
  ...
  Widget _buildPermissionBadge(ExternalToken token) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(hasTrade ? Icons.bolt : Icons.visibility_outlined,
              size: 10, color: color),
  ...
  Widget _buildAvatar(BuildContext context, ExternalToken token,
      {double radius = 20}) {
  ...
  Widget _buildEmptyState(BuildContext context, {bool isSearching = false}) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Icon(isSearching ? Icons.search_off : Icons.security,
