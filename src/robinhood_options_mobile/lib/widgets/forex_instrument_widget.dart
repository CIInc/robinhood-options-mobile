...
class _ForexInstrumentWidgetState extends State<ForexInstrumentWidget>
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.holding.quoteObj == null) {
      final pair = RobinhoodService.forexPairs.firstWhereOrNull(
        (element) =>
      ...
      futureQuote ??= widget.service.getForexQuote(
      ...
      body: FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            final historicalId = widget.holding.quoteObj?.id.isNotEmpty == true
  ...
  RefreshIndicator buildScrollView(ForexHolding holding, {bool done = false}) {
        ...
        builder: (BuildContext context, constraints) {
          ...
          return SliverAppBar(
            ...
            title: AppBarUtils.buildScrollToTopGestureDetector(
              ...
              child: Opacity(opacity: opacity, child: headerTitle(holding)),
            ),
            expandedHeight: 160,
            floating: false,
            pinned: true,
        ...
        child: Column(
          ...
          children: <Widget>[
            ListTile(
              ...
              trailing: Text(
                formatCurrency.format(holding.marketValue),
    ...
    if (!kIsWeb) {
      slivers.add(const SliverToBoxAdapter(child: SizedBox(height: 25.0)));
      slivers.add(
  ...
  void _startRefreshTimer() {
    ...
    refreshTriggerTime = Timer.periodic(const Duration(milliseconds: 15000), (
      ...
      if (widget.brokerageUser.refreshEnabled) {
        ...
        var updatedHolding = holdings.firstWhereOrNull(
          (element) => element.id == widget.holding.id,
        );
  ...
  Future<void> _generateAIContent(
    ...
    try {
      ...
      final response = await genService.generateContentFromServer(
        prompt,
        null,
        null,
        null,
          ...
          child: Padding(
            ...
            child: Card(
              ...
              child: ExpansionTile(
                ...
                initiallyExpanded: keys.values.any(
                  (key) =>
                      generativeProvider.promptResponses[key]?.isNotEmpty ==
                      true,
                ),
                children: [
                  Padding(
  ...
  void _openAIChat(BuildContext context, String symbol) {
    final prompts = [
      Prompt(
        ...
        prompt:
            'Summarize the forex pair or currency $symbol, including its '
      ...
      Prompt(
        ...
        title: '$symbol Trend',
        prompt:
            'Analyze the technical trend for forex $symbol and describe '
      ...
      Prompt(
        ...
        prompt:
            'Discuss potential risk-aware trading approaches for forex '
  ...
  Widget quoteWidget(ForexHolding holding) {
    return SliverToBoxAdapter(
      child: Column(
        ...
        children: <Widget>[
          ListTile(
            title: const Text("Quote", style: TextStyle(fontSize: 20.0)),
            trailing: Text(
  ...
  SingleChildScrollView _buildQuoteScrollRow(
    ...
    String? todayReturnPercentText = formatPercentage.format(
      todayReturnPercent,
    );
                ...
                children: [
                  ...
                  Text(
                    formatPercentage.format(
                      holding.quoteObj!.changePercentToday,
                    ),
                    style: TextStyle(
                      fontSize: 12.0,
                      color: holding.quoteObj!.changeToday > 0
                  ...
                  Text(
                    "${holding.quoteObj!.changeToday > 0
                        ...
                        : holding.quoteObj!.changeToday < 0
                        ? "-"
                        : ""}${holding.quoteObj!.markPrice! < 0.001 ? NumberFormat.simpleCurrency(decimalDigits: 8).format(holding.quoteObj!.changeToday.abs()) : formatCurrency.format(holding.quoteObj!.changeToday.abs())}",
                    style: TextStyle(
                      fontSize: 12.0,
                      color: holding.quoteObj!.changeToday > 0
                          ? (Theme.of(context).brightness == Brightness.light
  ...
  SingleChildScrollView _buildDetailScrollRow(
    ...
    String? totalReturnPercentText = formatPercentage.format(
      totalReturnPercent,
    );
...
class _ForexChartWidgetState extends State<ForexChartWidget> {
  ...
  Widget build(BuildContext context) {
    if (widget.holding.historicalsObj != null &&
      ...
      final candles = _historicalsToCandles(
        ...
        for (final band in [
          ...
          seriesList.add(
            charts.Series<InstrumentHistorical, DateTime>(
              ...
              domainFn: (InstrumentHistorical history, _) => history.beginsAt!,
              measureFn: (_, index) => index != null && index < band.$2.length
                  ? band.$2[index]
                  : null,
              data: widget.holding.historicalsObj!.historicals,
            ),
          ...
          () {
            return provider.selection != null
                ? formatCompactDateTimeWithHour.format(
                    (provider.selection as InstrumentHistorical).beginsAt!
                        .toLocal(),
                  )
                : '0';
          },
  ...
  Widget _buildSelectedIndicatorValues(
    List<InstrumentHistorical> historicals,
    InstrumentHistorical selected,
    Color textColor,
  ) {
    final selectedIndex = historicals.indexWhere(
      (historical) => historical.beginsAt == selected.beginsAt,
    );
    ...
    if (_showVolume && historicals.any((h) => h.volume > 0)) {
      chips.add(
        buildChip(
          'Vol',
          formatCompactNumber.format(selected.volume),
          Theme.of(context).colorScheme.primary,
        ),
      );
    }
    ...
    for (final label in enabled) {
      ...
      final value = selectedIndex < values.length
  ...
  Widget _buildInlineTechnicalAnalysis(BuildContext context) {
    ...
    var sma200 = TechnicalIndicators.calculateSMA(
      sortedCandles,
      200,
    ).lastOrNull;
    ...
    var williamsR = TechnicalIndicators.calculateWilliamsR(
      sortedCandles,
    ).lastOrNull;
    var roc = TechnicalIndicators.calculateROC(sortedCandles).lastOrNull;
    var cmf = TechnicalIndicators.calculateChaikinMoneyFlow(
      sortedCandles,
    ).lastOrNull;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildIndicatorWithSignal(
                      ...
                      valueText: sma10 != null ? _formatPrice(sma10) : null,
                      indicatorKey: 'sma',
                    ),
                    _buildIndicatorWithSignal(
                      "SMA 20",
                      sma20,
                      (v) => _comparePrice(currentPrice, v),
                      valueText: sma20 != null ? _formatPrice(sma20) : null,
                      indicatorKey: 'sma',
                    ),
                    _buildIndicatorWithSignal(
                      "SMA 50",
                      sma50,
                      (v) => _comparePrice(currentPrice, v),
                      valueText: sma50 != null ? _formatPrice(sma50) : null,
                      indicatorKey: 'sma',
                    ),
                    _buildIndicatorWithSignal(
                      "SMA 200",
                      sma200,
                      (v) => _comparePrice(currentPrice, v),
                      valueText: sma200 != null ? _formatPrice(sma200) : null,
                      indicatorKey: 'sma',
                    ),
                    _buildIndicatorWithSignal(
                      "EMA 12",
                      ema12,
                      (v) => _comparePrice(currentPrice, v),
                      valueText: ema12 != null ? _formatPrice(ema12) : null,
                      indicatorKey: 'ema',
                    ),
                    _buildIndicatorWithSignal(
                      "EMA 26",
                      ema26,
                      (v) => _comparePrice(currentPrice, v),
                      valueText: ema26 != null ? _formatPrice(ema26) : null,
                      indicatorKey: 'ema',
                    ),
                    _buildIndicatorWithSignal(
                      "VWAP",
                      vwap,
                      (v) => _comparePrice(currentPrice, v),
                      valueText: vwap != null ? _formatPrice(vwap) : null,
                      indicatorKey: 'vwap',
                    ),
                    const Divider(),
                    ...
                    _buildIndicatorWithSignal(
                      ...
                      valueText: atr != null ? _formatPrice(atr) : null,
                      indicatorKey: 'atr',
                    ),
                    _buildIndicatorWithSignal(
                      "OBV",
        ...
        children: [
          ...
          Flexible(
            ...
            child: Row(
              ...
              children: [
                Text(
                  valueText ?? (value != null ? _formatPrice(value) : "N/A"),
  ...
  List<Candle> _generateCandles(
    ...
    for (int i = 0; i < historicals.length; i += bucketSize) {
      ...
      double low = chunk
          .map(
            (e) => math.min(
              e.lowPrice ?? double.infinity,
              e.openPrice ?? double.infinity,
            ),
          )
          .reduce(math.min);
