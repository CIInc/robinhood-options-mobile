...
class _LoginWidgetState extends State<LoginWidget> {
  ...
  void initState() {
    ...
    final initialIndex = widget.initialSource != null
        ? _brokerageOptions.indexWhere(
            (option) => option['source'] == widget.initialSource,
          )
        : 0;
  ...
  void _setSelectedBrokerage(BrokerageSource selected, [int? index]) {
    ...
    if (nextIndex >= 0) {
      ...
      if (_carouselController.hasClients && _brokerageItemExtent > 0) {
        final targetOffset = (nextIndex * _brokerageItemExtent).clamp(
          0.0,
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
          ...
          builder: (context, AsyncSnapshot<http.Response> authenticationSnapshot) {
            debugPrint(authenticationSnapshot.connectionState.toString());
            if (authenticationSnapshot.data != null) {
              var authenticationResponse = jsonDecode(
                authenticationSnapshot.data!.body,
              );
              debugPrint(jsonEncode(authenticationResponse));
              if (authenticationResponse['challenge'] != null) {
                challengeRequestId = authenticationResponse['challenge']['id'];
                challengeType =
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) async {
                  ...
                  if (!popped) {
                    widget.analytics.logLogin(
                      loginMethod: "Robinhood $challengeType",
                    );
                    if (context.mounted) {
                ...
                if (authenticationSnapshot.connectionState ==
                  ...
                  if (authenticationResponse['error_description'] != null) {
                    errorMessage = authenticationResponse['error_description'];
  ...
  void _login() async {
    if (source == BrokerageSource.demo ||
      ...
      if (authResult['verification_workflow'] != null) {
        ...
        var userMachineResponse = await service.userMachine(
          deviceToken!,
          workflowId,
        );
  ...
  void _handleChallenge() async {
    if (challengeRequestId != null) {
      ...
      var challengeResponse = await service.respondChallenge(
        challengeRequestId!,
        smsCtl.text.isEmpty ? mfaCtl.text : smsCtl.text,
      );
  ...
  void _startMonitoringClipboard() {
    ...
    clipboardTriggerTime = Timer.periodic(const Duration(milliseconds: 500), (
      timer,
    ) {
  ...
  void _startPollingPromptChallenge() {
    _promptPollTimer?.cancel();
    _promptPollTimer = Timer.periodic(const Duration(seconds: 2), (
      timer,
    ) async {
      if (challengeRequestId == null) {
        timer.cancel();
