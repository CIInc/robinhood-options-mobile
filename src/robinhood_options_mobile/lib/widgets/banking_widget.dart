...
class _BankingWidgetState extends State<BankingWidget>
  ...
  Future<(List<AchTransfer>, List<AchRelationship>)> _fetchData() async {
    try {
      ...
      if (widget.service is RobinhoodService) {
        ...
        transfersFuture = widget.service.getAchTransfersModel(
          widget.brokerageUser,
        );
        relationshipsFuture = widget.service.getAchRelationshipsModel(
          widget.brokerageUser,
        );
      }
  ...
  Future<void> _cancelTransfer(AchTransfer transfer) async {
    ...
    final success = await widget.service.cancelAchTransfer(
      widget.brokerageUser,
      transfer.cancelUrl!,
    );
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Transfer of ${transfer.formattedAmountPlain} cancelled.',
          ),
          ...
          child: Row(
            children: [
              ...
              _buildChoiceChip(
                ...
                onSelected: () {
                  setState(() {
                    _directionFilter = _directionFilter == 'deposit'
                        ? 'all'
                        : 'deposit';
                  });
                },
              ...
              _buildChoiceChip(
                ...
                onSelected: () {
                  setState(() {
                    _directionFilter = _directionFilter == 'withdraw'
                        ? 'all'
                        : 'withdraw';
                  });
                },
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              CircleAvatar(
                radius: 20,
                backgroundColor: transfer.directionColor.withValues(
                  alpha: 0.15,
                ),
                child: Icon(
                  transfer.directionIcon,
      ...
      child: Center(
        child: Column(
          children: [
            ...
            const SizedBox(height: 12),
            Text('No transfers found', style: theme.textTheme.titleMedium),
            const SizedBox(height: 4),
            Text(
          ...
          builder: (_, scrollController) {
            return ListView(
              ...
              children: [
                ...
                _buildDetailRow(
                  'Initiated Date',
                  transfer.formattedCreatedDateTime,
                ),
                if (transfer.formattedLandingDate != null)
                  _buildDetailRow(
                    'Clearing / Arrival',
                    transfer.formattedLandingDate!,
                  ),
                _buildDetailRow(
                  'Transfer Fees',
                  _currencyFormat.format(transfer.fees),
                ),
