...
void main() {
  ...
  group('short stock', () {
    test('sell with no position opens a short (150% initial check, '
    ...
    test(
      'selling more than a long position is rejected (no silent flip)',
      () async {
        final store = makeStore();
        final instrument = makeInstrument();
        await store.submitStockOrder(
  ...
  group('cash-secured puts', () {
    test(
      'sell-to-open a put reserves strike x 100 and credits the premium',
      () async {
        final store = makeStore();
  ...
  group('covered calls', () {
    ...
    test(
      'assignment: expired ITM call calls the shares away at the strike',
      () async {
        final store = makeStore();
        final instrument = makeInstrument();
        await store.submitStockOrder(
  ...
  group('long option lifecycle unchanged', () {
    ...
    test(
      'shorting does not disturb an existing long in another contract',
      () async {
        final store = makeStore();
