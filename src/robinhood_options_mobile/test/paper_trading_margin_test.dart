...
void main() {
  ...
  group('processMarginCalls', () {
    test('healthy account is untouched', () async {
      ...
      final liquidated = await store.processMarginCalls(
        stockPrices: {'AAPL': 100.0},
      );
    ...
    test(
      'deficit triggers a partial buy-to-cover that restores margin',
      () async {
        final store = await storeWithShort();
    ...
    test('blown account liquidates everything and records a warning', () async {
      ...
      final warning = store.history.firstWhere(
        (h) => h['type'] == 'MARGIN',
        orElse: () => {},
      );
    ...
    test('covers the largest exposure first across multiple shorts', () async {
      ...
      final tsla = store.positions.firstWhere(
        (p) => p.instrumentObj?.symbol == 'TSLA',
      );
      expect(tsla.quantity, -10); // untouched
      final aapl = store.positions.firstWhere(
        (p) => p.instrumentObj?.symbol == 'AAPL',
      );
