...
void main() {
  group('InsiderSentimentSummary & Models', () {
    test(
      'parses full insider summary with monthly breakdown and transactions',
      () {
        final summaryJson = {
          'status': 'SUCCESS',
          'data': {
  ...
  group('DemoService Insider Endpoints', () {
    ...
    test('returns realistic insider data for AAPL', () async {
      final summaryRes = await demoService.getInsiderSummary(
        user,
        'inst_aapl_01',
      );
      final txRes = await demoService.getInsiderTransactions(
        user,
        'inst_aapl_01',
      );
      ...
      expect(summary.isBullish, isTrue);
      expect(
        summary.transactions.any((t) => t.filerName == 'Tim Cook'),
        isTrue,
      );
    ...
    test(
      'returns realistic insider data for TSLA (executive selling)',
      () async {
        final summaryRes = await demoService.getInsiderSummary(
          user,
          'inst_tsla_01',
        ...
        expect(
          summary.transactions.any(
            (t) => t.filerName.contains('Robyn Denholm'),
          ),
          isTrue,
        );
      },
