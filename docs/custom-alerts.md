...
- **News Alerts**: Action Center alerts for active portfolio holdings affected by high-impact news catalysts (e.g. FDA decisions, regulatory investigations, enterprise partnerships), rapid 24h sentiment shifts (>= 15 pts surge or drop), and extreme sentiment regimes (very bearish or very bullish), detailing held multi-asset exposure (shares and option contracts) with 1-tap navigation to the Insights section and AI news intelligence breakdown.
- **Unusual Activity Alerts**: Action Center alerts for active portfolio holdings and option contracts detecting volume surges and abnormal price movements: heavy-volume selloffs (confluence of >= 2.0x volume multiple and sharp decline <= -4%), high-volume breakouts (confluence of >= 2.0x volume multiple and sharp gain >= +4%), extreme unusual volume (>= 2.5x 30-day average volume indicating pending catalyst or institutional repositioning), sharp standalone intraday price swings (>= +6% rally or <= -6% drop), and abnormal option contract volume (daily contract volume >= 2.0x or 3.0x open interest with minimum 500 contract threshold), with 1-tap navigation to Positions.
- **Congressional Trading Alerts**: Smart-alert rules can match disclosed congressional purchases, sales, or any qualifying transaction against a configured dollar threshold. Action Center alerts also identify portfolio overlap and link to the Congress Trading research view; see [Congress & Political Trading Tracker](congress-trading-tracker.md) for data and filing-delay details.
